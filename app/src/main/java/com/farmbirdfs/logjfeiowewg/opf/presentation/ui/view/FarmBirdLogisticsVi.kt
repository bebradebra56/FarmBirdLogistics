...
class FarmBirdLogisticsVi(
    private val farmBirdLogisticsContext: Context,
    private val farmBirdLogisticsCallback: FarmBirdLogisticsCallBack,
    private val farmBirdLogisticsWindow: Window
) : WebView(farmBirdLogisticsContext) {
    private var farmBirdLogisticsFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun farmBirdLogisticsSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.farmBirdLogisticsFileChooserHandler = handler
    }
    init {
        ...
        webSettings.apply {
            ...
            userAgentString = WebSettings.getDefaultUserAgent(farmBirdLogisticsContext).replace("; wv)", "").replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        farmBirdLogisticsContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(farmBirdLogisticsContext, "This application not found", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
            }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("https://test-web.syndi-test.net") != true && url?.contains("https://web.team-s.club") != true) {
                    FarmBirdLogisticsApplication.farmBirdLogisticsInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(FarmBirdLogisticsApplication.FARM_BIRD_LOGISTICS_MAIN_TAG, "onPageFinished : ${FarmBirdLogisticsApplication.farmBirdLogisticsInputMode}")
                    farmBirdLogisticsWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    farmBirdLogisticsEnableStableInputScroll()
                } else {
                    FarmBirdLogisticsApplication.farmBirdLogisticsInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE
                    Log.d(FarmBirdLogisticsApplication.FARM_BIRD_LOGISTICS_MAIN_TAG, "onPageFinished : ${FarmBirdLogisticsApplication.farmBirdLogisticsInputMode}")
                    farmBirdLogisticsWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                farmBirdLogisticsFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            override fun onCreateWindow(
                ...
                farmBirdLogisticsHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    fun farmBirdLogisticsFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun farmBirdLogisticsEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    private fun farmBirdLogisticsHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView = FarmBirdLogisticsVi(farmBirdLogisticsContext, farmBirdLogisticsCallback, farmBirdLogisticsWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            farmBirdLogisticsCallback.farmBirdLogisticsHandleCreateWebWindowRequest(windowWebView)
        }
    }
